...
export const HowItWorks: React.FC = () => {
  ...
  return (
    <section className="py-20 relative overflow-hidden bg-muted">
      ...
      <div className="absolute inset-0 bg-gradient-to-b from-muted via-muted to-green-100/40 pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {steps.map((step) => (
            <div key={step.id} className="bg-background p-6 rounded-2xl shadow-sm border border-border relative group hover:shadow-xl hover:shadow-green-100/50 transition-all duration-300 transform hover:-translate-y-1">
              <div className="flex items-center gap-3 mb-6">
                <span className="text-6xl font-extrabold text-cico-green opacity-90 tracking-tighter leading-none">
                  {step.id}.
                </span>
                <div className="p-3 bg-green-50 rounded-xl group-hover:bg-cico-green group-hover:text-white transition-colors duration-300">
                  {React.cloneElement(step.icon as React.ReactElement, {
                    className: "w-6 h-6 text-cico-green group-hover:text-white transition-colors"
                  })}
                </div>
              </div>
